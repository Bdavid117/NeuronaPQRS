...
const URGENCIA_COLORS: Record<string, string> = {
  alta:  "text-red-700 bg-red-50 border-red-200 dark:text-red-300 dark:bg-red-900/30 dark:border-red-700/30",
  media: "text-amber-700 bg-amber-50 border-amber-200 dark:text-amber-300 dark:bg-amber-900/30 dark:border-amber-700/30",
  baja:  "text-green-700 bg-green-50 border-green-200 dark:text-green-300 dark:bg-green-900/30 dark:border-green-700/30",
};
...
export function ChatStream({ sessionId, onCaseUpdate, onAgentChange, initialPrompt, voiceMode = false }: ChatStreamProps) {
  ...
  return (
    <div className="flex flex-col h-full">
      ...
      {caseInfo && (
        <div className={cn(
          ...
          caseInfo.requiresHuman
            ? "bg-amber-50 border-amber-200 dark:bg-amber-950/50 dark:border-amber-700/50"
            : "bg-emerald-50 border-emerald-200 dark:bg-emerald-950/50 dark:border-emerald-700/50"
        )}>
          <div className="flex items-start gap-3">
            ...
            <div className="flex-1 min-w-0">
              <p className={cn("font-semibold text-sm mb-1", caseInfo.requiresHuman ? "text-amber-800 dark:text-amber-200" : "text-emerald-800 dark:text-emerald-200")}>
                {caseInfo.requiresHuman ? "Caso escalado para revisión humana" : "Caso radicado exitosamente"}
              </p>
              <div className="flex items-center gap-2 flex-wrap">
                <code className="text-xs font-mono text-slate-700 dark:text-white/80 bg-slate-100 dark:bg-white/10 px-2 py-0.5 rounded border border-slate-300 dark:border-white/15">
                  {caseInfo.radicado}
                </code>
                {caseInfo.tipo && (
                ...
                <button
                  ...
                  className="flex items-center gap-1 text-[10px] text-slate-500 dark:text-white/40 hover:text-slate-700 dark:hover:text-white/70 border border-slate-300 dark:border-white/15 rounded-md px-1.5 py-0.5 bg-white dark:bg-white/5 hover:bg-slate-50 dark:hover:bg-white/10 transition-colors"
                  title="Ver código QR"
                >
                  <QrCode size={10} />
                  QR
              ...
              {caseInfo.plazo && (
                <div className="flex items-center gap-1.5 text-xs text-slate-500 dark:text-white/50 mt-1.5">
                  <Clock size={11} />
                  <span>Plazo: {caseInfo.plazo}</span>
                </div>
              )}
              {showQR && (
                <div className="mt-3 p-2 bg-slate-50 dark:bg-white/10 rounded-lg border border-slate-200 dark:border-white/10 inline-block">
                  <QRCodeSVG
                    value={`${typeof window !== "undefined" ? window.location.origin : ""}/r/${caseInfo.radicado}`}
                    size={96}
                    level="M"
                  />
                  <p className="text-[9px] text-slate-400 dark:text-white/40 text-center mt-1">Escanea para ver tu caso</p>
                </div>
              )}
            </div>
          </div>
      ...
      {pendingAttachments.length > 0 && (
        <div className="mx-4 mb-2 flex gap-1.5 flex-wrap">
          {pendingAttachments.map((id) => (
            <span key={id} className="text-xs bg-indigo-50 dark:bg-indigo-500/15 text-indigo-700 dark:text-indigo-300 border border-indigo-200 dark:border-indigo-500/25 rounded-full px-2.5 py-1 flex items-center gap-1.5">
              <Paperclip size={10} />
              Archivo #{id}
              <button
      ...
      {voiceMode ? (
        /* ── Voice mode ── */
        <div className="px-4 pb-6 pt-2 flex flex-col items-center gap-3 border-t border-slate-200 dark:border-white/[0.07]">
          <div className="flex items-center gap-4">
            {tts.isSpeaking && (
              <button
                onClick={tts.stop}
                className="p-2 rounded-full bg-slate-100 dark:bg-white/10 border border-slate-200 dark:border-white/15 text-slate-600 dark:text-white/60 hover:bg-slate-200 dark:hover:bg-white/15 transition-all"
          ...
          <p className="text-center text-xs text-slate-500 dark:text-white/40 max-w-xs leading-relaxed">
            {voiceState === "error"
              ? <span className="text-rose-500 dark:text-rose-400">Error de transcripción. Intenta de nuevo.</span>
              : voiceState === "listening" && voiceInterim
              ? <span className="text-slate-700 dark:text-white/60 italic">&ldquo;{voiceInterim}&rdquo;</span>
        ...
        <div className="px-4 pb-4 pt-1">
          {isFrustrated && (
            <div className="mb-2 flex items-center gap-2 text-xs text-amber-700 dark:text-amber-300 bg-amber-50 dark:bg-amber-900/30 border border-amber-300 dark:border-amber-500/40 rounded-xl px-3 py-2">
          ...
          <div className={cn(
            "flex items-end gap-2 bg-slate-50 dark:bg-white/[0.06] border rounded-2xl px-3 py-2.5 transition-all duration-200",
            isFrustrated
              ? "border-amber-400 dark:border-amber-500/50 focus-within:border-amber-500 dark:focus-within:border-amber-400"
              : "border-slate-200 dark:border-white/10 focus-within:border-primary-400 dark:focus-within:border-primary-400/60"
          )}>
            <button
              ...
              className={cn(
                ...
                showUpload
                  ...
                  : "text-slate-400 dark:text-white/30 hover:text-slate-600 dark:hover:text-white/60 hover:bg-slate-100 dark:hover:bg-white/[0.06]"
              )}
            ...
            <textarea
              ...
              rows={1}
              disabled={isLoading}
              className="flex-1 bg-transparent resize-none text-slate-900 dark:text-white placeholder-slate-400 dark:placeholder-white/30 text-sm outline-none py-0.5 max-h-32 overflow-y-auto leading-relaxed disabled:opacity-50"
